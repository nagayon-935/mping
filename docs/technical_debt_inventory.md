...
### TD-01 コミット済みビルド成果物 `main`（11.8MB）が git 追跡されている
...
### TD-02 CI に `-race` / `go vet` / lint / フォーマットチェックがない
...
### TD-03 `updateTable` の動的列処理が DNS 列を無視（`calcColumnWidths` と不整合）
...
### TD-04 HTTP ペインのログ追記関数のエビクション不整合
...
### TD-07 RTT 記録（min/max/sum/samples + リングバッファ）の三重実装
...
### TD-09 `updateTable` が毎 tick `GetView()`+`buildFullColumns` を各ターゲット2回呼ぶ
...
### TD-12 CLAUDE.md のアーキテクチャ記述が実態と乖離
...
### TD-19 設定配管の5点セット同期問題（フラグ/config/YAML/apply/validate）
...
### TD-20 `TraceRoute` と MTR プローブ経路の二重実装の統合
...
### TD-21 UI 列レイアウトの並行スライス+マジックインデックス構造
...
### TD-25 YAML ホットリロードで `port:` 変更が無視される
...
### TD-33 `tui_test.go`（2,800行）の分割
テーブル/ヘルパー/キー操作/リフレッシュ等でファイル分割したいが、テスト資産としては機能している。TD-21/23 のついでに移動する。

### TD-34 `wellKnownServices` マップ（80行）が tui_helpers.go に直書き
