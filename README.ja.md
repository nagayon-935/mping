...
* **ホストグループ**: YAML ファイルで名前付きグループを定義できます。グループごとにヘッダ行と最悪値集計行 (ロス数合計・RTT/Jitter 最大値・MinRTT 最小値) を表示します。`z` キーでグループの折りたたみ/展開を切り替えられます。
...
集計行の集計方法:
* **Sent / Recv / Loss** — グループ内全メンバーの合計
* **AvgRTT / MaxRTT / LastRTT / Jitter** — グループ内最悪値 (最大値)
* **MinRTT** — グループ内最良値 (最小値)
...
| **z** | 現在のスクロール位置にあるグループを折りたたみ/展開する (グループ定義時のみ有効) |
